...
vec3 getPixel(in vec2 coord, float time) {    
    ...
    // Use touch position for slight camera movement if available
    if (touch.x > 0.0 || touch.y > 0.0) {
        vec2 mouseUV = touch / resolution.xy;
...
void main() {
    ...
    vec3 color = getPixel(gl_FragCoord.xy, animTime);
